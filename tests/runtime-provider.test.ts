...
test("blank react vite catalog webapp repos get a framework-aware bootstrap", async () => {
  const server = createServer((_, res) => {
    res.setHeader("content-type", "application/json");
    res.end(JSON.stringify({ choices: [{ message: { content: "done" } }] }));
  });
  ...
  const cwd = await mkdtemp(join(tmpdir(), "doo-harness-runtime-provider-bootstrap-react-webapp-"));
  ...
  try {
    ...
    const runtime = await HarnessRuntime.create(cwd);
    await runtime.plan(
      "Build a React and Vite interactive clothing product promotional catalog webapp with a branded landing page, catalog listing, category/tag/season filters, product detail pages, related products, inquiry/interest CTA, URL-backed filter state, responsive layout, mock data, tests, and a README.",
      true
    );
    await runtime.advanceMilestone();
    const taskId = await runtime.executeCurrentTask();
    const packageBody = await readFile(join(cwd, "package.json"), "utf8");
    const mainBody = await readFile(join(cwd, "src", "main.jsx"), "utf8");
    const appBody = await readFile(join(cwd, "src", "App.jsx"), "utf8");
    const testBody = await readFile(join(cwd, "tests", "catalog.test.js"), "utf8");
    await execFileAsync("pnpm", ["run", "test"], {
      cwd,
      env: {
    ...
    assert.match(appBody, /CatalogApp/);
    assert.match(appBody, /filtersToQuery/);
    assert.match(testBody, /submitInquiryLead/);
  } finally {
    server.close();
...
test("blank nextjs catalog webapp repos get a framework-aware bootstrap", async () => {
  const server = createServer((_, res) => {
    res.setHeader("content-type", "application/json");
    res.end(JSON.stringify({ choices: [{ message: { content: "done" } }] }));
  });
  ...
  const cwd = await mkdtemp(join(tmpdir(), "doo-harness-runtime-provider-bootstrap-next-webapp-"));
  ...
  try {
    ...
    const runtime = await HarnessRuntime.create(cwd);
    await runtime.plan(
      "Build a Next.js interactive clothing product promotional catalog webapp with a branded landing page, catalog listing, category/tag/season filters, product detail pages, related products, inquiry/interest CTA, URL-backed filter state, responsive layout, mock data, tests, and a README.",
      true
    );
    await runtime.advanceMilestone();
    const taskId = await runtime.executeCurrentTask();
    const packageBody = await readFile(join(cwd, "package.json"), "utf8");
    const pageBody = await readFile(join(cwd, "app", "page.jsx"), "utf8");
    const layoutBody = await readFile(join(cwd, "app", "layout.jsx"), "utf8");
    const clientBody = await readFile(join(cwd, "app", "catalog-client.jsx"), "utf8");
    const testBody = await readFile(join(cwd, "tests", "catalog.test.js"), "utf8");
    await execFileAsync("pnpm", ["run", "test"], {
      cwd,
      env: {
    ...
    assert.equal(taskId, "T2");
    assert.match(packageBody, /"next"/);
    assert.match(packageBody, /"start": "next dev"/);
    assert.match(layoutBody, /Catalog Layout/);
    assert.match(pageBody, /CatalogClient/);
    assert.match(clientBody, /filtersToQuery/);
    assert.match(testBody, /submitInquiryLead/);
  } finally {
    server.close();
    await rm(cwd, { recursive: true, force: true });
